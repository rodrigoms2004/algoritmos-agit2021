...
#include "sorting_insertsort.hpp"
...
namespace sorting_insertsort
{
    ...
    static void bench_vec_insertsort(picobench::state& s)
    {
        std::vector<int> xs = get_singleton_test_vector(s.iterations());
        ...
        auto it = xs.begin();
        auto it2 = xs.begin(); std::advance(it2, xs.size() - 1);
        std::cout << std::setw(7) << s.iterations() << "   bench_vec_insertsort: "
            << std::setw(2) << *it << " " << std::setw(7) << *it2 << "\n";
    }

    static void register_benchmarks()
    {
        std::vector<int> ITERS{ 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536 };

        picobench::global_registry::set_bench_suite("insertsort");
        picobench::global_registry::new_benchmark("bench_vec_std_sort", bench_vec_std_sort).iterations(ITERS);
        picobench::global_registry::new_benchmark("bench_vec_insertsort", bench_vec_insertsort).iterations(ITERS).baseline();
    }
    ...
    {
        {
            ...
            internal::display(xs, " before insertsort");
            sorting_insertsort::insertsort(xs);
            internal::display(xs, "  after insertsort");
        }
        {
...
{
    using sorting_insertsort::test_interactive;
    using sorting_insertsort::register_benchmarks;
