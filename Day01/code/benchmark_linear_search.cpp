// g++ -O3 benchmark_linear_search.cpp -o benchmark_linear_search.o
...
std::vector<int>& get_sigleton_test_vector()
{
  static std::vector<int> xs = generate_random_ints(1000000, 1, 1000); // run once only
  return xs;
}
...
{
  //... initialization
  std::vector<int>& xs = get_sigleton_test_vector();
  int target = 42;
  // int target = 1001; // worse case, not found
  s.start_timer();
...
{
  std::vector<int> ITERS { 1 << 10, 1 << 11, 1 << 12, 1 << 13, 1 << 14, 1 << 15 }; // 1 << 10 == 2^10 (1024) ... 2^11 (2048)
